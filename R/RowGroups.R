...
RowGroups = function(x,returnGroups=FALSE, returnGroupsId=FALSE){
  ...
  if(NROW(x)==0) 
    ...
    #return(RowGroupsOld(x=x,returnGroups=returnGroups, returnGroupsId=returnGroupsId))

  xInteger = AsFactorInteger(x)
  if(!is.null(xInteger)){
    if(NCOL(xInteger)==1){
      ix <-  order(xInteger)           #sort(xInteger,index.return = TRUE)$ix
  ...
  #cumsum(!dp[ix])
  a <- rep(NA_integer_,length(dp))    # rep(NaN,length(dp))
  a[ix] <- cumsum(!dp[ix])
...
AsFactorInteger =
  function (x)
  {
    #for (i in seq_len(NCOL(x))) x[, i] <- as.integer(as.factor(x[, i])) -1
    ...
    #k=rev(rev(c(1,cumprod(as.numeric( apply(x, MARGIN=c(2), max)+1))))[-1])
...
Match  = function (x,y)
{
  ...
  if(any(!(names(x) == names(y)))) # Kan skrives om for å spare minne
...
MatchOld =   ################ matcher rader
  function (x,y)
  {
    ...
    if(a-b != 2)
      stop("The problem is to big to be handled by the function Match")
