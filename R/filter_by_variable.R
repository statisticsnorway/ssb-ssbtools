#' Filter a List of Items by a Variable
#'
#' Filters a list of items, retaining only those associated with a specific variable.
#' The association between items and variables is provided via a named list, where
#' each element contains a vector of variables corresponding to an item in `items`.
#'
...
#'
#' @return A named list containing a subset of `items` where each element is associated 
#'   with the specified `variable`. If no matches are found, an empty list is returned.
#' 
...
#'
#' # Filter by variable "y"
#' filter_by_variable("y", items, variable_mapping)
#'
#' # Filter by a variable that does not exist
