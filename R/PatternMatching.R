...
HierarchicalWildcardGlobbing = function(z,wg,useUnique=NULL,useFactor=FALSE, makeWarning=TRUE, printInfo = FALSE,
                                        useMatrixToDataFrame = TRUE){
  #starttime = Sys.time()
  if(any( !(names(wg) %in% c("sign",names(z))  ))){
    if(makeWarning) warning("wg: Unique variant of common variables used")
  ...
  if(any( !( c("sign",names(z)) %in% names(wg)))){
    if(makeWarning) warning("z: Common variables used (unique variant)")
    z = z[(c("sign",names(z)) %in% names(wg))[-1]]
    if(is.data.frame(z))
      z = unique(z)
  }

  # Endrer rekkefølge i wg til samme rekkefølge som dataene i z
  # Noe kode under forutsetter det (crossMerge)
  # Burde vært samkjørt med det over
  ...
  for(i in 1:length(z)){
    ...
    #if(is.data.frame(z))
    if(useFactor)
      x[[i]][[1]] = factor(unique(x[[i]][[1]]))
    else
      x[[i]] = CharacterDataFrame(unique(x[[i]]))
  }



  #useUnique=FALSE lønner seg med kostradata , men det var før
  # 146 sekunder mot 234 sekunder

  #useFactor=FALSE lønner seg også
  # 155 mot 239 sekunder
  ...
  #cat("\n ",names(z),names(wg),"nGroups =",nGroups,"\n")
  #flush.console()
  ...
  for(i in 1:length(z)){
    #print(names(z)[i])
    #print(rg$groups$sign)
    for(j in seq_len(nGroups)[as.logical(rg$groups$sign*rg$groups[,names(z)[i]])])
      #print(j)
  ...
  # Reduserer hver variabel med negative sign (kun grupper med en variabel)
  #if(FALSE){
  for(i in 1:length(z)){
    #print(names(z)[i])
    for(j in seq_len(nGroups)[as.logical((!rg$groups$sign)*rg$groups[,names(z)[i]])])
  ...
  if(FALSE){
    # variant som bare setter sammen
  ...
  for(i in seq_len(NROW(hc$merge))){
    ...
    if(is.data.frame(z)){ ## TAr bare med det som finnes i z
      #print("MAtch")
      #print(names(y[[i]]))
    ...
    # Må plukke ut rader i rg$groups[,-signNr] som ikke er med i usedGroups
    # og hvor alle TRUE er blant variablene i y[[i]]


    newGroups = which(rowSums((rg$groups[,-signNr]+0)[, !(names(rg$groups[,-signNr]) %in% names(y[[i]])),
                                                      drop=FALSE])==0 & !usedGroups)
    # bruker disse
    #cat("\n newGroups: ",newGroups, "\n")
    if(length(newGroups)>0){

      usedGroups[newGroups]=TRUE
      # rekkefølge: tar alle TRUE først og starter med de med færres true
      # de med signNr FALSE kommer til slutt
      ...
      for(j in newGroups){
        ...
        if(length(vars)<dim(y[[i]])[2] & useUniqueHere){
          ...
          cat("]")
          flush.console()
          selg = WildcardGlobbing(rgy$groups,
                                  wg[rg$idx==j,vars,drop=FALSE], sign = rg$groups[j,signNr])
          sel = selg[rgy$idx]
        }
        else{
          sel = WildcardGlobbing(y[[i]][,vars,drop=FALSE],
                                 wg[rg$idx==j,vars,drop=FALSE], sign = rg$groups[j,signNr])
        }
...
WildcardGlobbing = function(x,wg,sign=TRUE,invert="!"){
  ...
  for(j in matlabColon(2,nCOLwg))
    selijLast[[j]] = selijLast[[1]]
  for(i in 1:NROW(wg)){
    ...
    for(j in 1:nCOLwg){
      if(!(wg[i,j]==wgijLast[j])){
        ...
        if(length(wgss)==1)
          selijLast[[j]] = grepl(glob2rx(wgss) , x[,names(wg)[j]])
        else
          selijLast[[j]] = !grepl(glob2rx(wgss[2]) , x[,names(wg)[j]])
      } #else
        #cat("æ")
