...
DataFrameToMatrix = function(x){
  ...
  if(any(!(classDF %in% c("character","factor","integer","logical","numeric"))))
    warning(paste("Classes not treated. MatrixToDataFrame() will not work:",
                  paste(unique(classDF[!(classDF %in% c("character","factor","integer","logical","numeric"))]),collapse=", ")))
  
  
  #numericx = sapply(codeFrame1,is.numeric)
  #factorx  = sapply(codeFrame1,is.factor)
...
MatrixToDataFrame = function(x,forceStringsAsFactors=FALSE){
  ...
  if(any(!(classDF %in% c("character","factor","integer","logical","numeric"))))
    stop(paste("Classes not treated:",
               paste(unique(classDF[!(classDF %in% c("character","factor","integer","logical","numeric"))]),collapse=", ")))
