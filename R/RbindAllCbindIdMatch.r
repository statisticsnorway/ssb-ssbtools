...
CbindIdMatch <-  function(...,addName=names(x),sep="_",
  ...
  for(i in matlabColon(2,n)){
    if(!is.na(idNames[i]) & !is.na(idNames1[i])){
      ...
      if(NROW(x[[i]])==1) rows[,i] = 1 
      else if(NROW(x[[i]])!=m) stop("Element of idNames/idNames1 missing when needed")
    }
  }
  for(i in seq_len(n)) {
    if(!is.na(addName[i])) 
  ...
  #eval(parse(text = paste("cbind(",paste("x[[",seq_len(n),"]][rows[,",seq_len(n),"], ,drop=FALSE],",collapse = ""),"deparse.level = 0)")))
  x = eval(parse(text = paste("cbind(",paste("x[[",seq_len(n),"]][rows[,",seq_len(n),"],",cols[seq_len(n)],",drop=FALSE],",collapse = ""),"deparse.level = 0)")))
